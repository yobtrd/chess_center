...
class ReportController:
    ...
    def generate_tournaments_list_console_reports(self):
        ...
        Displays the list of finished tournament.
        ...
        while True:
            ...
            tournament_choice = (
                self.view.get_report_tournament_choice(ended_tournaments))
    ...
    def main_html_file(self):
        ...
        links = [
            ("liste_joueurs_enregistres.html",
             "Liste des joueurs enregistrés"),
            ("liste_tournois_precedent.html",
             "Liste des tournois précédents")
        ]
        items = [
            f'<li><a href="data/reports/{file}">{text}</a></li>'
            for file, text in links
        ]
    ...
    def generate_players_list_content(self):
        ...
        return "\n".join(
            ...
            f"({p['birthdate']}) | INE: {p['chess_id']}</li>"
    ...
    def generate_tournaments_content(self):
        ...
        for tournament in ended_tournament:
            ...
            items.append(
                f'<li><a href="{file_path}">{title}</li>'
            )
            html_file = (
                self.generate_individual_tournaments_content(tournament, title)
                )
    ...
    def generate_rounds_and_matches_html(self, tournament):
        ...
        for round in tournament["tournament_rounds_list"]:
            ...
            for i, match in enumerate(round["matches"], 1):
                ...
                html.append(
                    f"<li>"
                    f"Match {i}: {player1} ({p1_points}) "
                    f"VS. {player2} ({p2_points})"
                    f"</li>"
                )
            html.append("</ul>")
        return "\n".join(html)
    ...
    def get_alphabetical_player_list(self, list):
        ...
        return sorted(
            list, key=lambda p: (p["last_name"], p["first_name"])
        )

    def get_tournament_players_details(self, tournament):
        """Retrieves all information about a player via their chess ID."""
        player_ids = {
            p["chess_id"] for p in tournament["tournament_players_list"]
            }
        return [
            p for p in self.player_controller.saved_players_list
            if p["chess_id"] in player_ids
        ]
    ...
    def checks_for_ended_tournaments(self):
        ...
        return [
            t for t in saved_tournaments if t["tournament_datas"]["end_date"]
            ]
